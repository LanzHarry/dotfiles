...
`.editorconfig` should also live in the project root and be committed to any
source control. Any overlapping settings should match between `.editorconfig`
and the formatting config files defined for the formatters.
...
This is done by setting `capabilities.documentFormattingProvider = false` (and
`documentRangeFormattingProvider = false`) in the LSP config, which tells the
client not to use the server's formatting capability at all. The nvim client
here is specifically negotiating what LSP capabilities are desired, the language
server also advertises its capabilities to the client when attaching.
